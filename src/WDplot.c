...
int PlotWaveforms(void)
{
	...
	while(ntr > 0) {
		...
		for(i=0; i<PlotVar.NumTraces; i++) {
			if (s < PlotVar.TraceSize[i]) {
				if (PlotVar.DataType == PLOT_DATA_UINT16) {
                    uint16_t *data = (uint16_t *)PlotVar.TraceData[i];
				    fprintf(fplot, "%d\t", data[s]-2096);
	...
	//fprintf(gnuplot, ", 10000*exp(-x/8.68/2) with lines linecolor 3\n");
	fprintf(gnuplot, "\n"); 
	fflush(gnuplot);

	/* set the time gnuplot will have finished */
	WaitTime = npts/20;
	if (WaitTime < PlotVar.TimeDelay)
		WaitTime = PlotVar.TimeDelay;
